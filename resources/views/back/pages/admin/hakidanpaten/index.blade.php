                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"><a
                                href="{{ route('admin.hakidanpaten.index') }}">@yield('title')</a>
                        </li>
                    </ol>
                </div>
            </div>
        ...
        <div class="container-fluid">
            <div class="row justify-content-end">
                <div class="col-2 my-3">
                    <a href="{{ route('admin.hakidanpaten.create') }}"
        ...
        <script type="text/javascript">
            $(function() {
                ...
                var table = $('.hakidanpaten-datatable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('admin.hakidanpaten.list') }}",
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'DT_RowIndex'
                        },
                ...
                $(document).on('click', '.hapus', function() {
                    if (window.confirm('Are you sure?')) {
                        let id = $(this).attr('id')
                        $.ajax({
                            url: "{{ route('admin.hakidanpaten.hapus') }}",
                            type: 'post',
                            data: {
                                id: id,
                                "_token": "{{ csrf_token() }}"
                            ...
                            success: function(params) {
                                alert(params.text)
                                $('.hakidanpaten-datatable').DataTable().ajax.reload()
                            }
                        })
                    }
                })
