                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"><a
                                href="{{ route('admin.penelitian-dosen.index') }}">@yield('title')</a></li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            <div class="row mx-auto">
                                <div class="col-3 my-3">
                                    <a href="{{ route('admin.penelitian-dosen.create') }}"
        ...
        <script type="text/javascript">
            $(function() {
                ...
                var table = $('.penelitian-datatable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('admin.penelitian-dosen.list') }}",
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'DT_RowIndex'
                        },
                ...
                $(document).on('click', '.hapus', function() {
                    if (window.confirm('Are you sure?')) {
                        let id = $(this).attr('id')
                        $.ajax({
                            url: "{{ route('admin.penelitian-dosen.hapus') }}",
                            type: 'post',
                            data: {
                                id: id,
                                "_token": "{{ csrf_token() }}"
                            ...
                            success: function(params) {
                                alert(params.text)
                                $('.penelitian-datatable').DataTable().ajax.reload()
                            }
                        })
                    }
                })
