                ...
                <div class="card-body">
                  <div class="row mx-auto">
                    <div class="col-3 my-3">
                      <a href="{{ route('superadmin.settings.user.create') }}" class="btn btn-block btn-primary btn-sm">Tambah</a>
                    </div>
                  </div>
                  {{-- {{ $dataTable->table() }} --}}
                  <div class="row mx-auto">
...
<script type="text/javascript"> 
  $(function () {
    ...
    var table = $('.user-datatable').DataTable({
        ...
        ajax: "{{ route('superadmin.settings.user.listUser') }}",
        columns: [
    ...
    var tableAdmin = $('.admin-datatable').DataTable({
        processing: true,
        serverSide: true,
        buttons: [
        'copy', 'excel', 'pdf'
          ],
        ajax: "{{ route('superadmin.settings.user.listAdmin') }}",
        columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: 'name', name: 'name'},
            {data: 'email', name: 'email'},
            {data: 'prodi.name', name: 'prodi.name', orderable: false},
            {
                data: 'action', 
                name: 'action', 
                orderable: false, 
    ...
    $(document).on('click','.hapus', function () {
        let id = $(this).attr('id')
        $.ajax({
            url : "{{route('superadmin.settings.user.hapus')}}",
            type : 'post',
            data: {
                id: id,
                "_token" : "{{csrf_token()}}"
