                ...
                <div class="card-body">
                  <div class="row mx-auto">
                    <div class="col-3 my-3">
                      <a href="{{ route('humas.manajemen-berita.kategori.create') }}" class="btn btn-block btn-primary btn-sm">Tambah</a>
                    </div>
                  </div>
                  {{-- {{ $dataTable->table() }} --}}
                  <div class="row mx-auto">
...
<script type="text/javascript"> 
  $(function () {
    ...
    var table = $('.kategori-datatable').DataTable({
        ...
        ajax: "{{ route('humas.manajemen-berita.kategori.list') }}",
        columns: [
    ...
    $(document).on('click','.hapus', function () {
        let id = $(this).attr('id')
        $.ajax({
            url : "{{route('humas.manajemen-berita.kategori.hapus')}}",
            type : 'post',
            data: {
                id: id,
                "_token" : "{{csrf_token()}}"
            ...
            success: function (params) {
                alert(params.text)
                $('.kategori-datatable').DataTable().ajax.reload()
            }
        })
      })
  });
